...
Функция ЕстьКодВСтроке(Знач пСтрокаКода, предСтрокаЗакончиласьПереносом)
	...
	ключевыеСлова = КлючевыеСловаНачалаНеКода();
	
	Для каждого цИсключение Из ключевыеСлова Цикл
	...
	Если предСтрокаЗакончиласьПереносом Тогда
		...
		естьКодВСтроке = СтрНайти( строкаКода, "(" );
...
Функция СтрокаЗаканчиваетсяПереносом( Знач пСтрока )
	...
	Если ( СтрНачинаетсяС( пСтрока, "ПРОЦЕДУРА" )
		ИЛИ СтрНачинаетсяС( пСтрока, "ФУНКЦИЯ" ) )
		И СтрЗаканчиваетсяНа( пСтрока, ")" ) Тогда
	...
	словаПереноса = КлючевыеСловаПереноса();
	
	Для каждого цСлово Из словаПереноса Цикл
		
		позицияСлова = СтрНайти( пСтрока, цСлово, НаправлениеПоиска.СКонца );
		
		Если СтрЗаканчиваетсяНа( пСтрока, цСлово )
			И ЭтоОтдельноеСлово( пСтрока, цСлово, позицияСлова ) Тогда
